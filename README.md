...
1. **Elementwise gate dimensions have a near-binary structure** -- 48.3% are permanently inactive regardless of input, and 54.8% actively respond to input. This within-head structure is not in the original paper.
...
3. **Layer 0's gate actively modulates first-token attention** -- removing it reduces rather than restores first-token attention scores. The naive hypothesis that Layer 0's gate suppresses the attention sink is falsified.
...
Forcing Layer 0's gate open **reduces** first-token attention from 0.0526 to 0.0091 -- that's a -10.5% change relative to the gated baseline, meaning it moves further from the baseline, not closer. Forcing all gates open does the same thing (0.0086).

**Interpretation:** Layer 0's gate is not passively suppressing the attention sink. It actively directs attention toward the first token. Removing it makes first-token attention worse, not better. The mechanism is unclear, but the behaviour is non-trivial and not reported in the original paper.
...
| Always-closed dimensions (mean <0.1) | 48.32% |
| Always-open dimensions (mean >0.9) | 0.03% |
| Dynamic dimensions (variance >0.01) | 54.84% |

Note: The original paper reports headwise mean=0.172 from their training checkpoint. Our value of 0.2559 is from the released 3.5T-token model. Different checkpoint, consistent direction (gates are mostly closed).

The key finding in the elementwise model is that dimensions are not uniformly sparse. Nearly half (48.3%) are permanently inactive regardless of input. Over half (54.8%) actively respond to input. This within-head structure, some dimensions permanently off, others dynamically switching, is not reported in the original paper at any level.
...
Investigates whether the 48.3% permanently closed dimensions in the elementwise model are structured or random. The dimensions that are permanently closed are not randomly distributed, they show consistent patterns across layers and heads.
...
Tests whether the 48.3% permanently closed dimension set is stable across random splits of the data. The set of closed dimensions is highly consistent between halves, confirming this is a stable property of the model, not a sampling artifact.
...
|---------|-----------|----------|
| Elementwise gate dimensions have near-binary structure (48.3% permanently closed, 54.8% dynamic) | **High** | Welford aggregation over 25,600 tokens, split-half stability confirmed (Exp 12) |
...
| Layer 0 gate actively modulates first-token attention (not passive suppression) | **Moderate** | Attention weight hooks, but mechanism unclear |
